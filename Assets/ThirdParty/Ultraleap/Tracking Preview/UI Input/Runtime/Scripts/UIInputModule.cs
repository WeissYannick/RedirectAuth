...
namespace Leap.Unity.InputModule
{
    ...
    public class UIInputModule : BaseInputModule, IInputModuleEventHandler
    {
        ...
        [SerializeField] public HandModelManager handModelManager;
        ...
        public override void Process()
        {
            ...
            var activeHand = handModelManager.GetAllActiveRegisteredHands()[0].GetLeapHand();
            if (activeHand == null) return;
            if (activeHand.IsLeft)
            {
                _pointerLeft.Process(activeHand, _projectionOriginProvider);
            }
            else if (activeHand.IsRight)
            {
                _pointerRight.Process(activeHand, _projectionOriginProvider);
            }
        }
